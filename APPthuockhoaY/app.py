...
# ==========================================
def load_data():
    df_k = pd.read_csv(FILE_KHO) if os.path.exists(FILE_KHO) else pd.DataFrame(columns=BASE_COLS_KHO)
    for col in BASE_COLS_KHO:
        if col not in df_k.columns: df_k[col] = 0 if col in ['Nhập Mới', 'Đã Xuất'] else ""
    
    if not df_k.empty:
        df_k = df_k.sort_values(by='Thành Phần', ascending=True).reset_index(drop=True)

    if os.path.exists(FILE_NHAN_SU):
        df_ns = pd.read_csv(FILE_NHAN_SU)
        for col in BASE_COLS_NS:
            if col not in df_ns.columns: df_ns[col] = ""
    else:
        df_ns = pd.DataFrame([{'Username': 'DoanhoikhoaYNTTU', 'Password': '298300A', 'Quyền': 'admin', 'Họ Tên': 'Nguyễn Hữu Thọ (Win)', 'MSSV': '16012000', 'Lớp': 'Y20'}])

    df_ct = pd.read_csv(FILE_CHUONG_TRINH) if os.path.exists(FILE_CHUONG_TRINH) else pd.DataFrame([{'Tên Chương Trình': 'Kho Tổng', 'Trạng Thái': 'Đang mở'}])
    df_ls = pd.read_csv(FILE_LICH_SU) if os.path.exists(FILE_LICH_SU) else pd.DataFrame(columns=['Thời Gian', 'Chương Trình', 'Nơi Xuất', 'Tên Thuốc', 'Số Lượng'])
    df_dt = pd.read_csv(FILE_DU_TRU) if os.path.exists(FILE_DU_TRU) else pd.DataFrame(columns=['Tên Thuốc', 'Số Lượng Dự Trù'])
    df_nhom = pd.read_csv(FILE_NHOM_THUOC) if os.path.exists(FILE_NHOM_THUOC) else pd.DataFrame({'Tên Nhóm': ["Tim mạch", "Tiêu hóa", "Hô hấp", "Dị ứng", "Giảm đau", "Khác"]})
    return df_k, df_ls, df_ns, df_ct, df_dt, df_nhom
